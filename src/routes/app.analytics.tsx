...
export default function AnalyticsPage() {
  ...
  // Estimated focus hours logged from real completed tasks
  ...
  // Real-time weekly activity breakdown dynamically calculated from state
  const now = new Date();
  const weekStart = startOfWeek(now, { weekStartsOn: 1 });
  const dayNames = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  
  const weeklyData = dayNames.map((dayName, idx) => {
    const dayDate = addDays(weekStart, idx);
    const dayKey = format(dayDate, "yyyy-MM-dd");
    
    const dayTasks = state.tasks.filter((t) => {
      const taskDay = t.dueDate?.slice(0, 10);
      const completedDay = t.completedAt?.slice(0, 10);
      return taskDay === dayKey || completedDay === dayKey;
    });
    
    const dayDoneTasks = dayTasks.filter((t) => t.status === "done");
    const dayTasksCount = dayDoneTasks.length;
    const dayMinutes = dayDoneTasks.reduce((acc, t) => acc + (t.estimatedMinutes || 60), 0);
    const dayHours = dayMinutes / 60;
    
    const baseScore = dayTasks.length > 0 ? Math.round((dayDoneTasks.length / dayTasks.length) * 70) : (idx <= 4 ? 60 : 45);
    const habitBonus = state.habits.filter((h) => h.log.includes(dayKey)).length * 10;
    const dayScore = Math.min(100, Math.max(40, baseScore + habitBonus));
