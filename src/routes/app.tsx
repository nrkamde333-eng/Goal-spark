...
export function AppLayout() {
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`p-1 rounded-lg transition-colors ${active ? "bg-primary/20 text-primary" : "text-muted-foreground group-hover:text-foreground"}`}
              >
                <n.icon
                  className={`size-4.5 ${active ? "text-primary" : n.glow}`}
                />
              </div>
              <span>{n.label}</span>
            </div>
  ...
  return (
    <div className="h-screen flex bg-background overflow-hidden selection:bg-primary/30">
      {/* Sidebar */}
      <aside className="hidden md:flex w-64 shrink-0 flex-col border-r bg-sidebar text-sidebar-foreground sticky top-0 h-screen z-30 overflow-y-auto">
        <Link
          to="/"
          className="flex items-center gap-2.5 px-5 h-16 border-b border-border/50"
        >
          ...
          <div className="flex flex-col">
            <span className="font-display font-extrabold text-lg tracking-tight bg-linear-to-r from-foreground via-foreground to-primary bg-clip-text text-transparent">
              GoalPilot
            </span>
          ...
          <div className="rounded-2xl p-3.5 bg-muted/20 border border-border/60 shadow-xs">
            ...
            <div className="space-y-2.5">
              {state.goals.slice(0, 3).map((g) => {
                const catTheme =
                  CATEGORY_THEMES[g.category] || CATEGORY_THEMES.study;
                return (
                  <div key={g.id} className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="truncate max-w-33.75 font-medium text-foreground">
                        {g.title}
                      </span>
                      <span
                        className={`font-bold text-[11px] ${catTheme.textColor}`}
                      >
                        {g.progress}%
                      </span>
                    </div>
                    <div className="h-1.5 rounded-full bg-muted/60 overflow-hidden">
                      <div
                        className={`h-full bg-linear-to-r ${catTheme.gradient} transition-all duration-500 rounded-full`}
      ...
      <div className="flex-1 min-w-0 min-h-0 flex flex-col">
        <header className="sticky top-0 z-40 h-16 border-b bg-card flex items-center justify-between px-4 md:px-6 gap-3 shrink-0">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              ...
              <SheetContent
                side="left"
                className="w-64 bg-sidebar text-sidebar-foreground p-0 border-r flex flex-col"
              >
            ...
            <div className="relative w-full max-w-lg hidden sm:block">
              ...
              <Input
                placeholder="Search tasks, habits, roadmaps…"
                className="pl-9 rounded-full bg-muted/30 border-border/50 focus-visible:ring-primary/40"
              />
            </div>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            <Dialog open={quickOpen} onOpenChange={setQuickOpen}>
              <DialogTrigger asChild>
                <Button
                  size="sm"
                  className="rounded-full gradient-brand text-white hidden sm:inline-flex shadow-md shadow-primary/25 hover:shadow-lg hover:shadow-primary/35 transition"
                >
                        ...
                        return (
                          <button
                            ...
                            className={`text-xs px-2.5 py-1 rounded-lg border font-medium transition ${
                              selected
                                ? `${theme.badgeClass} ring-2 ${theme.ringColor}`
            ...
            <Button
              ...
              onClick={() => {
                resetDemo();
                toast.success("Refreshed with multi-domain demo state!");
              }}
