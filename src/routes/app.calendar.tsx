...
export function CalendarPage() {
  const { state, addEvent, deleteEvent, toggleTask } = useApp();
                ...
                >
                  ...
                  <div className="space-y-1">
                    {es.slice(0, 3).map((e) => {
                      ...
                      const linkedTask = e.taskId ? state.tasks.find(t => t.id === e.taskId) : undefined;
                      const isDone = linkedTask?.status === "done";
                      return (
                        <div
                          key={e.id}
                          className={`text-[10px] px-1.5 py-0.5 rounded font-medium truncate ${theme.calendarBadge} ${isDone ? "opacity-50 line-through" : ""}`}
                        >
                  ...
                  {e.taskId && (
                    <button
                      onClick={() => {
                        toggleTask(e.taskId!);
                        if (!isDone) {
                          toast.success(`Completed "${e.title.slice(0, 20)}..." · Progress updated!`);
                        } else {
                          toast.info(`Reopened "${e.title.slice(0, 20)}..."`);
                        }
                      }}
                    ...
                    >
                      {isDone ? (
                        ...
                        <Circle className="size-4 text-muted-foreground hover:text-primary transition" />
                      )}
                    </button>
                  )}
                  <div className="flex-1 min-w-0">
                    <div className={`text-sm font-medium truncate ${isDone ? "line-through text-muted-foreground" : "text-foreground"}`}>
                      {e.title}
                    </div>
