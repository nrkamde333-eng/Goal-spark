...
export function Dashboard() {
  ...
  const todayProgress = todayTasks.length > 0 ? Math.round((done / todayTasks.length) * 100) : 100;
  ...
  const handleTaskToggle = (t: any) => {
    ...
    toggleTask(t.id);
    if (willBeDone) {
      toast.success(`Completed "${t.title.slice(0, 24)}..." · Roadmap progress updated!`);
    } else {
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-muted-foreground">Here's your live execution and roadmap velocity today.</p>
        </div>
        ...
        <Card className="lg:col-span-2 border overflow-hidden">
          <CardHeader className="pb-3 flex-col gap-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono font-semibold text-primary">{done}/{todayTasks.length} done ({todayProgress}%)</span>
            </div>
            {todayTasks.length > 0 && (
