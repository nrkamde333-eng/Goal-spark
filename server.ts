...
// Helper to safely clean and extract JSON from model responses
function safeExtractJson(raw: string): any {
  ...
  // Strip code fences if present (e.g. ```json ... ``` or ``` ...)
...
// Resilient Gemini generateContent caller with model fallback & retry for 503/429
  ...
  for (let i = 0; i < candidateModels.length; i++) {
    ...
    try {
      ...
      if (isOverloadedOrUnavailable && i < candidateModels.length - 1) {
        console.warn(`[Gemini API] Model ${model} is busy/unavailable. Retrying with fallback model ${candidateModels[i + 1]}...`);
        // Brief jitter delay before fallback
        await new Promise((res) => setTimeout(res, 250));
        continue;
      }

      console.warn(`[Gemini API] Failed on model ${model}:`, errMsg);
    }
  }
...
Respond ONLY with valid JSON matching this schema:
{
  ...
  "tips": string[]
      ...
      const geminiResult = await callGeminiWithFallback(ai, {
      ...
      const json = safeExtractJson(geminiResult.text);
      if (!json || !json.weeklyCurriculum || json.weeklyCurriculum.length === 0) {
        return res.json(generateCustomDynamicPlan(goal));
      }
      res.json(json);
    } catch (err: any) {
      console.warn("[Plan Generation] Using local fallback plan:", err?.message || err);
...
4. QUICK PROMPTS:
   ...
   - "What should I do next?": Give immediate prioritization steps.`;
      ...
      res.json({ content: (geminiResult && geminiResult.text) ? geminiResult.text : getAssistantQuickResponse(lastMsg) });
    } catch (err: any) {
      console.warn("[Chat Assistant] Using assistant fallback response:", err?.message || err);
      res.json({
        content: getAssistantQuickResponse(lastMsg),
      });
    }
